...
query = Article.find(:first, :order => "updated_at DESC");
if query.nil?
...
page.events.each do |e|
  ...
  if ie.updated_time > lutdatabase
    ...
    edatabase = Article.find(:first, :conditions => [ "identifier = ?", ie.identifier ]);
    if edatabase
      puts "***found database entry, updating the entry "+edatabase.title
      edatabase.title = ie.name
      edatabase.description = ie.description
      edatabase.updated_at = ie.updated_time
      edatabase.identifier = ie.identifier
      edatabase.img_url = ie.picture
      edatabase.save
      puts "***updated "+edatabase.name
    else
      puts "***database entry not found, adding to database "+ie.name
      edatabase = Article.new( :title => ie.name,  :description => ie.description, :updated_at => ie.updated_time, :identifier => ie.identifier, :img_url=> ie.picture )
      edatabase.save
